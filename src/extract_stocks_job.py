...
def create_glue_catalog(database: str, table: str, s3_path: str) -> None:
    ...
    try:
        try:
            ...
            logger.info(f"Table '{database}.{table}' removed to recreate with correct schema.")
        except ClientError:
            pass  
        ...
        logger.info(f"Table '{database}.{table}' created in Glue Catalog with compatible schema.")
...
@staticmethod
def add_partition_if_not_exists(database: str, table: str, partition_value: str, partition_location: str) -> None:
    """
    ...
    try:
        ...
        logger.info(f"Partition dataproc={partition_value} added to table {database}.{table}")
    except ClientError as e:
        if e.response['Error']['Code'] == 'AlreadyExistsException':
...
def main():
    try:
        ...
        try:
            ...
            logger.info(f"Found {existing_df.count()} records in existing partition. Replacing...")
        except:
